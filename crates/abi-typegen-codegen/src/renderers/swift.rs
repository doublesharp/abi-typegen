...
pub fn render_swift_file(ir: &ContractIr) -> String {
    render_swift_file_with_wrappers(ir, false)
}

/// Renders Swift bindings, optionally including web3swift contract wrappers.
pub fn render_swift_file_with_wrappers(ir: &ContractIr, wrappers: bool) -> String {
    ...
    let mut event_type_names = Vec::new();
    let mut error_type_names = Vec::new();
    ...
    for (function, overload) in ir.functions.iter().zip(overloads) {
        ...
        if function.inputs.is_empty() {
            function_param_names.push(None);
            continue;
        }
        let fields = fields_of(
            function
        ...
        function_param_names.push(Some(params_name));
    }
    ...
    for (event, index) in ir.events.iter().zip(event_overloads) {
        ...
        event_type_names.push(type_name);
    }
...
mod tests {
    ...
    #[test]
    fn wrapper_mode_adds_typed_sdk_operations_and_preserves_metadata_mode() {
        let ir = parse_artifact("Token", r#"{"abi":[{"type":"function","name":"balanceOf","inputs":[{"name":"owner","type":"address"}],"outputs":[{"name":"balance","type":"uint256"}],"stateMutability":"view"},{"type":"function","name":"transfer","inputs":[{"name":"to","type":"address"},{"name":"amount","type":"uint256"}],"outputs":[{"name":"ok","type":"bool"}],"stateMutability":"nonpayable"}]}"#).expect("valid abi");
        let plain = render_swift_file(&ir);
        assert_eq!(plain, render_swift_file_with_wrappers(&ir, false));
        assert!(!plain.contains("public struct Client"));
        let wrapped = render_swift_file_with_wrappers(&ir, true);
        assert!(wrapped.contains("public struct Client"), "{wrapped}");
        assert!(
            wrapped.contains("public static func encodeBalanceOf"),
            "{wrapped}"
        );
        assert!(
            wrapped.contains("public static func decodeBalanceOfResult"),
            "{wrapped}"
        );
        assert!(wrapped.contains("public func readBalanceOf"), "{wrapped}");
        assert!(wrapped.contains("public func prepareTransfer"), "{wrapped}");
    }
